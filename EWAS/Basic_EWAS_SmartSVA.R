...
#Use SmartSVA to remove celltype effects
##Estimate number of SVs
Z = model.matrix(~ age + sex + condition, data = phenotype)
Z0 = model.matrix(~ age + sex, data = phenotype)

Y.r = t(resid(lm(t(betas) ~ Z)))
n.sv = EstDimRMT(Y.r, FALSE)$dim + 1
#8 svs estimated

SVs = smartsva.cpp(betas, Z, mod0 = Z0, 
                   B = 5, alpha = 1, VERBOSE = T,
                   n.sv = n.sv)

Z.SV = model.matrix(~ age + sex + condition + SVs$sv, data = phenotype)

#Categorical analysis

getinds = grep('condition',colnames(Z.SV))

fit = lmFit(log(betas/(1-betas)), design = Z.SV)
...
#dir.create('EWAS')
save(results,file=file.path('EWAS',"basic_EWAS_limma_SmartSVA.RData"))

#get predictions of difference for modal / average values of covariates
contrmat = matrix(0, nrow = ncol(Z.SV), ncol = 2)
colnames(contrmat) = c('normal','HS')  
rownames(contrmat) = colnames(Z.SV)

contrmat[-4,] = colMeans(Z.SV)[-4]
...
save(results2,file=file.path('EWAS',"basic_EWAS_limma_SmartSVA_absdiff.RData"))
